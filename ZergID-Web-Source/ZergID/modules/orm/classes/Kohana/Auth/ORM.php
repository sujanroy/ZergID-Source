...
class Kohana_Auth_ORM extends Auth {
	...
	protected function _login($user, $password, $remember)
	{
		if ( ! is_object($user))
		{
			...
			$user->where($user->unique_key($username), '=', $username)->and_where('is_deleted','=',1)->and_where('is_active','=',NULL)->find();
		}
	...
	public function logout($destroy = FALSE, $logout_all = TRUE)
	{
		...
		if ($token = Cookie::get('authautologin'))
		{
			...
			if ($token->loaded() AND $logout_all)
			{
				...
				//DB::query(Database::DELETE, 'DELETE FROM zid_user_tokens WHERE user_id ='.$token->user_id)->execute();
