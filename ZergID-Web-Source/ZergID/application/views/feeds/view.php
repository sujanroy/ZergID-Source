...
<div id="popup_box" class="popup_box">	<!-- OUR PopupBox DIV-->
     <?= Form::open('feeds/edit', array('enctype' => 'multipart/form-data')); ?>
    ...
    <div  class="feed" align="right">
        ...
        <div class="title"><?= HTML::anchor('user/others/'.$feed->id, ucfirst($feed->username)); ?></div>
         <?php else: ?>
         <div class="title"><?= HTML::anchor('user/others/'.$feed->id, ucfirst($feed->character_name)); ?> <span class="feed-charname"><?=$feed->username?>'s Character</span></div>
         <?php endif; ?>
         <?php 
...
<!--        <span id="bar_load" style="display:none; margin-left:15px;">?=HTML::image("public/images/loading-bar.gif", array('style'=>'margin-top:25px;'));?></span>
        -->
        <span style="float:right;margin-top:-14px"><?= Form::submit('game_submit', 'SUBMIT', array('class'=>'save-button', 'style'=>'width:115px; margin-top:20px', 'id'=>'post')); ?>   </span>
     <?= Form::close(); ?>
        <a id="popupBoxClose" class="media-box-close"><?=HTML::image("public/images/popup-close.gif");?></a>	
</div>
      ...
      <div style="float:left; margin-left:10px; margin-top:40px; margin-bottom: 25px;">
          <table style="width:580px; border-top: 1px solid #CECECE; border-bottom: 1px solid #CECECE; height: 55px;">
              <tr>
                  ...
                  <?php else: ?>
                  <a href="#" id="actual_zerg" class="zerg_count_blue" onclick="return zergitFeed('<?= URL::site();?>', '<?=$feed->feed_id?>', null)">Zerg it! <span style="color:#3A3A3A; margin-left: 5px;"><?php if($feed->zergs != NULL):  echo count(explode(",", $feed->zergs));  endif;?></span></a>
                  <?php endif; ?>
                  ...
                  <td><?= HTML::image("public/images/cmt-bar.png");?></td>
                  <td> <?php if($user->id == $feed->id): 
                      echo HTML::image("public/images/edit-icon.png", array('style'=>'margin-left:50px;', 'id'=>'feed_edit'));
                      echo HTML::image("public/images/delete-icon.png", array('style'=>'margin-left:30px;', 'onclick'=>'return deleteFeed("'.URL::site().'", "'.$feed->feed_id.'");')); ?>
                      <?php else: ?>
                      <?= Form::open('feeds/post_zerg_feed/', array('enctype'=>"multipart/form-data", 'style'=>'float:left')); ?>
                     <div id="postbox<?= $feed->feed_id; ?>" class="popup_box" style="top:110px; margin-left: -550px;"><!-- OUR PopupBox DIV-->
                    ...
                    <?php if($feed->parent_feed_id == "" || $feed->parent_feed_id == NULL): $parent_id = ""; else: $parent_id = $feed->parent_feed_id; endif; ?>
                     ...
                     <div style="float:left; width:100%;"><span id="barload<?=$feed->feed_id;?>" style="margin-left:15px; display: none;"><?= HTML::image("public/images/loading-bar.gif", array('style' => 'margin-top:25px;')); ?></span>
                         <?php
                                 ...
                                 endforeach;
                           ?>
                         <span style="float:right; margin-left:20px;"><?= Form::select('share_as', $character_list, '', array('class' => "search-select", 'id' => 'share_as'.$feed->feed_id, 'style' => 'width:140px; margin-right:5px')); ?>
                             <?= Form::submit('post_feed', 'POST', array('id' => 'post_feed', 'class' => 'save-button', 'onclick'=>"return postZergFeed('".URL::site()."', '".$feed->feed_id."', null, '".$feed->file_path."' , '".$feed->youtube_url."')")); ?></span></div>
...
<div style="margin-top:30px; width:480px;">
         ...
         <?php else: ?>
         <div class="title"><?= HTML::anchor('character/view/'.$comment->id, $comment->character_name); ?> <span class="feed-charname"><?= ucfirst($feed->username)?>'s Character</span></div>
         <?php endif; ?>
         <?php 
...
<?php endif; ?>
 ...
 <?php 
  ...
  if($user->id == $feed->id) {
      ...
      if($feed->character_detail_id != NULL):
         $character_id = $feed->character_detail_id;
      else:
         $character_id = 'null'; 
      endif;
  }
...
<?= Form::textarea('feedcmt', '', 
      ...
      <div class="hiddendiv cmt-input"></div>
           <?= Form::submit('post', 'POST', array('id'=>'cmt-button', 'alt'=>'reply_comment', 'class'=>'post-button-white', 'disabled', 'onclick'=>'return postComment("'.URL::site().'", '.$feed->feed_id.', '.$character_id.')')); ?>
         <?= Form::close(); ?>
  </div>
</div>
</div>
...
if(last_msg_id!='end'){
$.post('<?= URL::site(); ?>feeds/comment_more/',
{
lastmsg:last_msg_id,
feedid: feed_id,
beforeSend: function() { 
