...
//             echo $feed->original_user;
          ...
          if (!$feed->original_character){
            ...
            if(!$feed->original_character_icon):  
...
//        echo $feed->original_character_icon;
         ...
         <?php endif; ?>
         <?php
//   echo date('Y-m-d H:i:s', strtotime($feed->last_modified_date));
//  echo " feed1st ";
//  echo $current_date = date('Y-m-d H:i:s')." cur ";
...
//         endif; 
   ...
   //    echo     $feed_modified_date = $date->format('Y-m-d h:i A');
  //echo " 1st feed ";
  //          $currentdate = new DateTime($current_date);
  //          $currentdate->setTimeZone($tz);
  //     echo     $currentdate = $currentdate->format('Y-m-d h:i A');
...
//                 echo $feed->feed_rezerg_text;
             echo '<hr style="width:100%;height:1px">';
              ...
              <script> //script to append anchor tag,if some text & you tube url and some text posted(1st part text + youtube url + 2nd part text),normally the 2nd part of text is losing anchor tag focus,to get the correct feedid in anchor tag script placed here  
                $('p.text-link').contents().filter(function(){
                    return this.nodeType === 3;
                }).wrap(function(){
                    return $('<a/>', {href: "../../feeds/view/<?=$feed->feed_id; ?>"});
                });
               </script>
   ...
   <?php if($feed->file_path != "" || $feed->file_path != NULL): 
         ...
         <div><?php echo HTML::anchor("/feeds/view/$feed->feed_id",HTML::image($config_path->path['media'].$feed->file_path, array('class'=>'feed-media-image'))); ?></div>
         <?php else: ?>
         ...
         <div id="postbox<?= $feed->feed_id; ?>" class="popup_box" style="top:110px; margin-left: -220px;min-height: 280px;"><!-- OUR PopupBox DIV-->
        ...
        array('class'=>'feed_rezerginput','maxlength'=>'1000', 'rows'=>'3', 'cols'=>'50', 'placeholder'=>'Write something for your Zerg feed!', 'id'=>'feed_rezergtext'.$feed->feed_id,'style'=>'width:519px;margin-top:20px;resize:none')); ?>
                         ...
                         <?php else: ?>
                         <img  src="<?= URL::site();?>public/images/video_wrapper.jpg" style="height: 150px; width: 230px; position: absolute; margin-left: -531px; margin-top: 10px;" /><div style="float:right; width:290px; margin-top:15px;" class="feed-text"><?= $feed->feed_text;?></div>
                         <?php endif; ?>
                         <?php elseif($feed->youtube_url != "" || $feed->youtube_url != NULL):
              $embed_code = youtubeEmbedFromUrl($feed->youtube_url, 250, 170); ?>
             <img  src="http://i.ytimg.com/vi/<?=$embed_code; ?>/hqdefault.jpg" style="height: 150px; width: 230px; position: absolute; margin-left: -529px; margin-top: 90px;" /><div style="float:right; width:290px; margin-top:15px;" class="feed-text"><?= $feed->feed_text;?></div>
             <!--<img style="position:absolute;" src="/public/images/play-button.png" />-->
                     ...
                     <div style="float:left; width:100%;"><span id="barload<?=$feed->feed_id;?>" style="margin-left:250px; display: none;"><?= HTML::image("public/images/facebook_style_loader.gif", array('style' => 'margin-top:25px;')); ?></span>
                         <?php  $character_list[''] = "SHARE AS...";
                                ...
                                foreach ($characters as $character):
                                 ...
                                 endforeach;
                          ?>
                         <span style="float:right; margin-left:20px;margin-top: 50px;"><?= Form::select('share_as', $character_list, '', array('class'=>"select-input", 'id' => 'share_as'.$feed->feed_id, 'style' => 'width:140px; margin-right:5px; background-position:103px;')); ?>
         ...
         endif; if($feed->rezergs_count > 0): ?><span id="re_zerg<?= $feed->feed_id?>" style="color:#B8B8B8; margin-right:11px;float: right"><?= $feed->rezergs_count; ?> Re-Zergs</span><?php endif; ?>  
         </div>
<!--         <div class="cmt-box" id="cmt-<?= $feed->feed_id?>">
             <?php if($user->id == $feed->id):
                 if($feed->character->character_detail_id != NULL):
        ...
        array('class'=>'feed-comment','maxlength'=>'2000', 'rows'=>'1', 'placeholder'=>'Write your comment here!', 'id'=>'feed_cmt_'.$feed->feed_id, 'onkeyup'=>'return getCommentHeight('.$feed->feed_id.', this)')); ?>
           ...
           <input type="hidden" value="feed_cmt" id="chk-post-type" />
 <?= Form::close(); ?>  
         </div>-->
          </div>
          <?php if($i == "6" || $i == "11"):?>
    <div class="advertisment" style="margin-left:93px;max-height:70px">
...
<?php endif; endif; ?>
<script>
      function selectfeedtype(url,userId){
    var feed_type = $('#em'+userId).val();
        
        $.post(url+"user/select_feedtype", {
...
//           $('#popular_tab').css({'display':'inline'});
//           $('#recent_tab').css({'display':'none'});
//            } else if(response == '1'){
//                $('#popular_tab').css({'display':'none'});
//                $('#recent_tab').css({'display':'inline'});
//            }
